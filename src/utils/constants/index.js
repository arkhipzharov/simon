// If you changed/added/moved some icon, set value that is greater than previous
// to refresh icons cache. You can use floating point number that is as big as you want.
// If you don't see icon on website after this, to refresh icons cache:
// 1. disable webpack
// 2. set new value
// 3. enable webpack
export const ICONS_CACHE_REVISION = 1.5;

export const DIFFICULTIES_DATA = [
  {
    name: 'easy',
    timeBetweenClicksInSequenceMs: 1500,
  },
  {
    name: 'medium',
    timeBetweenClicksInSequenceMs: 1000,
  },
  {
    name: 'hard',
    timeBetweenClicksInSequenceMs: 400,
  },
];
